...
for i in nodes:
    tup=(options_8, i)
    val=graph_trial.edges()[tup]["Connection"]
    Connection.append(val)
...
if st.button("Apply Queery"):
    ...
    tb="Connection with "+ options_8
    #data_s["Number of Connection"]=Connection
    data_s[tb]=Connection
    data_s=data_s[["ID", tb, "Home Institution", "Country", "Position"]]
    st.dataframe(data_s)
